...
"""
    ...
    if needs_calculation and (has_docs or has_web):
        ...
        state["trace"].append({
            ...
            "output": "Calculation query",
            "handoff-to": "Table",
            "reasoning": "Query needs numeric extraction and calculation"
        })
        
        return {"context": context, "next_agent": "Table"}
    
    # Detect summary queries
...
"""
    ...
    try:
        ...
        if not result:
            ...
            if not is_relevant and not has_web:
                # Try web search if documents aren't relevant
                next_agent = "WebSearch"
            elif not is_relevant and has_web:
                # Both sources failed, send to Aggregator
                context["no_usable_data"] = True
                next_agent = "Aggregator"
            else:
                # Data is relevant, proceed to Aggregator
                next_agent = "Aggregator"
